...
func (s {{.Name}}) Insert() string {
	return "CREATE (:{{.Name}} { {{range $field := .Fields}}{{$field.Name}}: ${{$field.Name}},{{end}} })"	
}

func (s {{.Name}}) Get() string {
	return "match p=({{.Name}} { {{range $field := .Fields}}{{$field.Name}}: ${{$field.Name}},{{end}} })-[]->() return p"	
}
`
